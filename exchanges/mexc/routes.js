...
router.get("/account", async (req, res) => {
  try {
    ...
    const accountModule = require(path.resolve(
      __dirname,
...
router.get("/open-order", async (req, res) => {
  try {
    ...
    const openOrderModule = require(path.resolve(
      __dirname,
      "../../test/Trade/OpenOrder.js"
    ));
...
router.get("/assets-detail-futures", async (req, res) => {
  try {
    ...
    if (typeof assetsDetailModule.getAssetsDetails === "function") {
      try {
        const result = await assetsDetailModule.getAssetsDetails();
        res.json(result);
      } catch (error) {
        res.status(500).json(error);
      }
    ...
    console.error("Error importing AssetsDetailFutures.js:", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
